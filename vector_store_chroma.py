...
_DB_PATH = Path("./.chroma_db")
_DB_PATH.mkdir(exist_ok=True)


class ChromaVectorStore(VectorStore):
    def __init__(self) -> None:
        client = chromadb.PersistentClient(path=str(_DB_PATH), settings=Settings(allow_reset=True))
    ...
    @staticmethod
    def _split(
        chunks: Iterable[Tuple[str, dict]], embeddings: List[List[float]]
